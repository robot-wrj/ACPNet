...
__all__ = ['AverageMeter', 'evaluator', 'evaluator_p']
...
def evaluator_p(pred, gt):
    r""" Evaluation of decoding implemented in PyTorch Tensor
         Computes root mean squared error (RMSE).
    """
    # Calculate the RMSE
    # difference = gt - pred
    # squared_diff = difference[:, 0, :, :] ** 2 + difference[:, 1, :, :] ** 2
    # sum_squared_diff = torch.sum(squared_diff, dim=[1, 2])
    # rmse = torch.sqrt(torch.mean(sum_squared_diff))

    rmse = torch.sqrt(torch.mean((gt - pred)**2))

    # Calculate the MDE
    squared_diff = torch.sum(torch.square(gt - pred), dim=1)
    distance = torch.sqrt(squared_diff)
    mde = torch.mean(distance)
